...
```{r read_objects_from_file, eval=F}
...
final_viz_change_droppedNA <- final_viz_change_droppedNA %>%
 ...
 st_simplify(final_viz_change_droppedNA, dTolerance = 0.01, preserveTopology = TRUE)
# Create the map
tm <- tm_shape(final_viz_change_droppedNA) + 
     tm_polygons("change_cuba_fb", breaks = c(-1015, -500, 0, 500, 1847), 
...
#tm <- tm + tm_shape(metro_micro_boundaries_final) +
         # tm_borders(col = "red", lwd = .5, alpha = 0.3)  # Customize the boundary style

# Print the map
tm

tmap_save(tm, 'Capstone_int_Map_no_label.html')

```
```{r leaflet_test, echo=F}
tmap_leaflet(tm)
```
